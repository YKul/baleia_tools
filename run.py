...
from src.getseq import extract_seqs,process_isoforms
from src.alignment import align
from src import config,awarnach
...
RESULTS_DIRS = cfg.getResultsPath()
GENELIST_PATHS = cfg.getGenelistsPath()
GENOMICS_DIRS = cfg.getGenomesPath()
RESULTS_ISOFORMS = cfg.getIsoformsPath()
RESULTS_EXTRACTED = cfg.getExtractedPath()
RESULTS_FILTERED = cfg.getFilteredPath()
RESULTS_NAME_CHANGED_GENES = cfg.getNameChangedGenesPath()
RESULTS_MULTISPECIES_GENES = cfg.getMultispeciesGenesPath()
RESULTS_PROTEINS = cfg.getProteinsPath()
RESULTS_MULTISPECIES_PROT = cfg.getMultispeciesProteinsPath()
RESULTS_NUC_ALIGNMENTS = cfg.getNucAlignmentsPath()
CLUSTER_JOBS = cfg.getClusterJobsPath()
...
def extract_genes_of_interest(i):#,list_path: str):
    ...
    for filename in os.listdir(RESULTS_ISOFORMS[i]):
        ...
        with open(path_result, "r", encoding="ascii", errors="surrogateescape") as file:
            dataset_ie = extract_seqs.extract_genes(file, GENELIST_PATHS[i])
...
########################
#The configured path lists are parallelized by data group
#eg. RESULTS_DIRS[0] is for the first data group, etc.
#This program handles multiple data groups by counting through
...
for i in range(len(RESULTS_DIRS)):
    ...
    print("OK")
    print(f"Dataset {i}: Extracting longest isoforms...")
    #extract_longest_isoform(i,raw_data_file)
    print("OK")
    print(f"Dataset {i}: Extracting genes of interest from list...")
    #extract_genes_of_interest(i)#,GENELIST_PATHS[i])
    print("OK")
    print(f"Dataset {i}: Removing low quality sequences...")
    #remove_low_quality_sequences(i)
    print("OK")
    print(f"Dataset {i}: Editing sequence names...")
